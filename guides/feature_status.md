...
- `staging-live`: AppKit-backed, governed-effect-capable, and selected by an
...
| Run start | fixture-backed by default; staging-live for injected diagnostic lane; live-stack-deterministic in StackLab slice | `Synapse.AgentRuns.start_run/2` calls `AppKit.AgentIntake` with the fixture backend by default. With `:diagnostic_lane` and live-capable `EffectSurface` plus `AgentIntake` backends, it proposes a governed effect before starting the run. `mix stack_lab.synapse.live_slice --json` proves explicit AppKit -> Mezzanine AgentLoop start with runtime projection readback. |
...
| Governed-effect projection | staging-live for injected diagnostic lane | `Synapse.GovernedEffects` reads AppKit EffectSurface timelines and keeps denied effects as explicit product state. |
| Evidence/replay | fixture-backed by default; staging-live governed-effect refs when supplied by promoted run | UI evidence, receipt, replay, and missing evidence posture remain projected fixtures unless a promoted diagnostic run supplies governed-effect refs. |
