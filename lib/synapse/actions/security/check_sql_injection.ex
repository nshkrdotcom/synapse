defmodule Synapse.Actions.Security.CheckSQLInjection do
  ...
  defp analyze_diff(diff, files) do
    ...
    # Find lines with SQL injection patterns
    lines
    |> Enum.with_index()
  ...
  defp detect_sql_injection_in_line(line, _idx, files) do
    ...
    if length(matches) > 0 do
      # Determine which file (use first file as fallback)
  ...
  defp sql_injection_patterns do
    [
      # String interpolation in SQL - full patterns
      ~r/["']SELECT\s+.*\#{.*}.*["']/i,
      ~r/["']INSERT\s+.*\#{.*}.*["']/i,
      ~r/["']UPDATE\s+.*\#{.*}.*["']/i,
      ~r/["']DELETE\s+.*\#{.*}.*["']/i,
      # Simpler patterns for catching interpolation in SQL strings
      ~r/["'](SELECT|INSERT|UPDATE|DELETE|FROM|WHERE).*\#{/i,
      # WHERE clause with interpolation
      ~r/WHERE\s+.*=\s*["'].*\#{/i,
