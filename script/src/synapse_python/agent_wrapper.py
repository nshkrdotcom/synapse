...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        logger.error("Usage: python -m synapse_python.agent_wrapper <module_name>")
        sys.exit(1)

    module_name = sys.argv[1]
    try:
        # Import the module and get the agent instance
        logger.info(f"Attempting to import module: synapse_python.{module_name}")
        module = importlib.import_module(f"synapse_python.{module_name}")
    ...
    port = int(os.environ.get("SYNAPSE_PYTHON_AGENT_PORT", 8000))
...
# from synapse_python.agents.example_agent import agent as example_agent
...
# # #             return synapse_pb2.RunSyncResponse(
...
# # #                 yield synapse_pb2.RunResponseChunk(data=json.dumps(chunk).encode('utf-8'))
# # #         except Exception as e:
...
# # #         kind = 'request' if msg.kind == synapse_pb2.ModelMessage.REQUEST else 'response'
...
# # #     synapse_pb2_grpc.add_AgentServiceServicer_to_server(AgentServicer(agent_instances), server)
...
# #     async def RunSync(self, request: synapse_pb2.RunSyncRequest, context) -> synapse_pb2.RunSyncResponse:
# #         agent_id = request.agent_id
# #         if agent_id not in self.agent_instances:
...
# #     async def RunStream(self, request: synapse_pb2.RunRequest, context):
# #         agent_id = request.agent_id
# #         if agent_id not in self.agent_instances:
# #             raise Exception(f"Agent not found: {agent_id}")
...
# #     def _convert_message_to_pydantic_ai(self, msg: synapse_pb2.ModelMessage) -> ModelMessage:
# #         """Convert a gRPC ModelMessage to a pydantic-ai ModelMessage."""
# #         parts = []
# #         for part in msg.parts:
# #             if part.HasField("system_prompt_part"):
...
# #     def _convert_pydantic_ai_message_to_protobuf(self, msg: ModelMessage) -> synapse_pb2.ModelMessage:
# #         """Convert a pydantic-ai ModelMessage to a gRPC ModelMessage."""
# #         parts = []
# #         for part in msg.parts:
# #             if isinstance(part, SystemPromptPart):
# #                 parts.append(synapse_pb2.ModelMessagePart(system_prompt_part=synapse_pb2.SystemPromptPart(content=part.content)))
# #             elif isinstance(part, UserPromptPart):
# #                 parts.append(
# #                     synapse_pb2.ModelMessagePart(
# #                         user_prompt_part=synapse_pb2.UserPromptPart(
...
# #                 parts.append(
# #                     synapse_pb2.ModelMessagePart(
# #                         tool_return_part=synapse_pb2.ToolReturnPart(
...
# #                 parts.append(
# #                     synapse_pb2.ModelMessagePart(
# #                         retry_prompt_part=synapse_pb2.RetryPromptPart(
# #                             content=part.content, tool_name=part.tool_name, tool_call_id=part.tool_call_id
# #                         )
# #                     )
# #                 )
# #             elif isinstance(part, TextPart):
# #                 parts.append(synapse_pb2.ModelMessagePart(text_part=synapse_pb2.TextPart(content=part.content)))
# #             elif isinstance(part, ToolCallPart):
# #                 parts.append(
# #                     synapse_pb2.ModelMessagePart(
# #                         tool_call_part=synapse_pb2.ToolCallPart(
# #                             tool_name=part.tool_name,
...
# #         kind = synapse_pb2.ModelMessage.REQUEST if msg.kind == "request" else synapse_pb2.ModelMessage.RESPONSE

# #         return synapse_pb2.ModelMessage(kind=kind, parts=parts, timestamp=int(msg.timestamp.timestamp()))

# # ## FOR gRPC:
# # # def serve():
# # #     server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
# # #     synapse_pb2_grpc.add_AgentServiceServicer_to_server(AgentServicer(agent_instances), server)
# # #     server.add_insecure_port(f"[::]:{os.environ.get('SYNAPSE_PYTHON_AGENT_PORT', '50051')}")
# # #     server.start()
# # #     server.wait_for_termination()
...
# # # ```python
# # # python -m grpc_tools.protoc -I=./apps/synapse_python/src/synapse_python/protos --python_out=./apps/synapse_python/src/synapse_python/generated --pyi_out=./apps/synapse_python/src/synapse_python/generated --grpc_python_out=./apps/synapse_python/src/synapse_python/generated synapse.proto
# # # ````
